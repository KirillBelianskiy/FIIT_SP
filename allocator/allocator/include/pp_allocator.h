...
private:
    ...
    virtual void* do_allocate_sm(size_t, size_t);
...
template <typename T>
pp_allocator<T>::pp_allocator(std::pmr::memory_resource* mem) noexcept : _mem(
    mem == nullptr ? std::pmr::get_default_resource() : mem)
{
}
