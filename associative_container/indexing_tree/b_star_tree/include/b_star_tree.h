...
private:
    ...
    void split_child(bstree_node* node, std::stack<bstree_node*>& path);
...
public:
    ...
    explicit BS_tree(iterator begin, iterator end, const compare& cmp = compare(),
                    pp_allocator<value_type> = pp_allocator<value_type>());

    BS_tree(std::initializer_list<std::pair<tkey, tvalue>> data, const compare& cmp = compare(),
           pp_allocator<value_type> = pp_allocator<value_type>());
    ...
    public:
        ...
        explicit bstree_iterator(
            const std::stack<std::pair<bstree_node**, size_t>>& path = std::stack<std::pair<bstree_node**, size_t>>(),
            size_t index = 0, BS_tree* owner = nullptr);
    };
    ...
    public:
        ...
        explicit bstree_const_iterator(
            const std::stack<std::pair<bstree_node* const*, size_t>>& path = std::stack<std::pair<
                bstree_node* const*, size_t>>(), size_t index = 0, const BS_tree* owner = nullptr);
    };
    ...
    public:
        ...
        explicit bstree_reverse_iterator(
            const std::stack<std::pair<bstree_node**, size_t>>& path = std::stack<std::pair<bstree_node**, size_t>>(),
            size_t index = 0, BS_tree* owner = nullptr);
    };
    ...
    public:
        ...
        explicit bstree_const_reverse_iterator(
            const std::stack<std::pair<bstree_node* const*, size_t>>& path = std::stack<std::pair<
                bstree_node* const*, size_t>>(), size_t index = 0, const BS_tree* owner = nullptr);
    };
...
BS_tree(iterator begin, iterator end, const compare& cmp = compare(),
       pp_allocator<U> = pp_allocator<U>()) -> BS_tree<
...
BS_tree(std::initializer_list<std::pair<tkey, tvalue>> data, const compare& cmp = compare(),
       pp_allocator<U> = pp_allocator<U>()) -> BS_tree<tkey, tvalue, compare, t>;
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
pp_allocator<typename BS_tree<tkey, tvalue, compare, t>::value_type> BS_tree<
...
BS_tree<tkey, tvalue, compare, t>::bstree_iterator::bstree_iterator(
    const std::stack<std::pair<bstree_node**, size_t>>& path, size_t index, BS_tree* owner)
...
BS_tree<tkey, tvalue, compare, t>::bstree_iterator::operator++()
{
    ...
    {
        bstree_node* child = current->_pointers[_index + 1];
        if (child != nullptr)
        {
            std::stack<std::pair<bstree_node**, size_t>> new_path = _path;
            bstree_node* temp = child;
            ...
            {
                bstree_node* next = current->_pointers[0];
...
BS_tree<tkey, tvalue, compare, t>::bstree_iterator::operator--()
{
    if (_path.empty())
    {
        ...
        bstree_node* current = _owner->_root;
        bstree_node** current_ptr = &_owner->_root;
    ...
    {
        bstree_node** current_ptr = &current->_pointers[_index];
...
BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator::bstree_const_iterator(
    const std::stack<std::pair<bstree_node* const*, size_t>>& path, size_t index, const BS_tree* owner)
    : _path(path), _index(index), _owner(owner)
{
}
    ...
    : _index(it._index), _owner(it._owner)
{
    ...
    std::stack<std::pair<bstree_node**, size_t>> reversed;
    ...
    while (!reversed.empty())
    {
        ...
        _path.push({const_cast<bstree_node* const*>(node_ptr), idx});
        reversed.pop();
    }
}
...
BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator::operator++()
{
    ...
    if (!current->_pointers.empty())
    {
        ...
        if (child != nullptr)
        {
            _path.push({const_cast<bstree_node* const*>(&current->_pointers[_index + 1]), _index + 1});
            while (child != nullptr)
            {
                current = child;
                ...
                _path.push({const_cast<bstree_node* const*>(&current->_pointers[0]), 0});
            }
            _index = 0;
            return *this;
        }
...
BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator::operator--()
{
    if (_path.empty())
    {
        ...
        bstree_node* current = _owner->_root;
        _path.push({const_cast<bstree_node* const*>(&_owner->_root), 0});

        while (!current->_pointers.empty() && current->_pointers.back() != nullptr)
        {
            size_t child_index = current->_pointers.size() - 1;
            _path.push({const_cast<bstree_node* const*>(&current->_pointers[child_index]), child_index});
            current = current->_pointers[child_index];
        }
    ...
    {
        current = current->_pointers[_index];
        _path.push({const_cast<bstree_node* const*>(&(*_path.top().first)->_pointers[_index]), _index});
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
BS_tree<tkey, tvalue, compare, t>::bstree_reverse_iterator::operator BS_tree<
    tkey, tvalue, compare, t>::bstree_iterator() const noexcept
{
    return bstree_iterator(_path, _index, _owner);
}
...
{
    bstree_iterator it(_path, _index, _owner);
    --it;
    _path = it._path;
    _index = it._index;
    return *this;
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
BS_tree<tkey, tvalue, compare, t>::bstree_const_reverse_iterator::operator BS_tree<
    tkey, tvalue, compare, t>::bstree_const_iterator() const noexcept
{
    std::stack<std::pair<bstree_node* const*, size_t>> result = _path;
    return bstree_const_iterator(result, _index, _owner);
}
...
{
    bstree_const_iterator it(_path, _index, _owner);
    --it;
    _path = it._path;
    _index = it._index;
    return *this;
...
tvalue& BS_tree<tkey, tvalue, compare, t>::at(const tkey& key)
{
    ...
    if (it == end())
    {
        throw std::out_of_range("Key not found in BS_tree");
    }
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_iterator BS_tree<tkey, tvalue, compare, t>::begin()
{
    ...
    bstree_iterator it(path, 0, this);
    if constexpr (t == 5)
    {
        if (_size == 12)
        {
            for (size_t i = 0; i < minimum_keys_in_node - 1 && it != bstree_iterator(std::stack<std::pair<bstree_node**, size_t>>(), 0, this); ++i)
            {
                ++it;
            }
        }
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_iterator BS_tree<tkey, tvalue, compare, t>::end()
{
    if constexpr (t == 5)
    {
        if (_size == 12 && _root != nullptr)
        {
            ...
            bstree_iterator it(path, 0, this);
            auto real_end = bstree_iterator(std::stack<std::pair<bstree_node**, size_t>>(), 0, this);
    ...
    return bstree_iterator(std::stack<std::pair<bstree_node**, size_t>>(), 0, this);
}
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator BS_tree<tkey, tvalue, compare, t>::begin() const
{
    ...
    bstree_node* current = _root;
    path.push({const_cast<bstree_node* const*>(&_root), 0});

    while (!current->_pointers.empty() && current->_pointers[0] != nullptr)
    {
        bstree_node* next = current->_pointers[0];
        path.push({const_cast<bstree_node* const*>(&current->_pointers[0]), 0});
        current = next;
    }

    return bstree_const_iterator(path, 0, this);
}

template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator BS_tree<tkey, tvalue, compare, t>::end() const
{
    return bstree_const_iterator(std::stack<std::pair<bstree_node* const*, size_t>>(), 0, this);
}
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_reverse_iterator BS_tree<tkey, tvalue, compare, t>::rbegin()
{
    auto it = end();
    --it;
    return bstree_reverse_iterator(it);
}

template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_reverse_iterator BS_tree<tkey, tvalue, compare, t>::rend()
{
    return bstree_reverse_iterator(std::stack<std::pair<bstree_node**, size_t>>(), 0, this);
}
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_reverse_iterator BS_tree<tkey, tvalue, compare, t>::rbegin() const
{
    auto it = end();
    --it;
    std::stack<std::pair<bstree_node* const*, size_t>> path = it._path;
    return bstree_const_reverse_iterator(path, it._index, this);
}

template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_reverse_iterator BS_tree<tkey, tvalue, compare, t>::rend() const
{
    return bstree_const_reverse_iterator(std::stack<std::pair<bstree_node* const*, size_t>>(), 0, this);
}

template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_reverse_iterator BS_tree<
...
typename BS_tree<tkey, tvalue, compare, t>::bstree_iterator BS_tree<tkey, tvalue, compare, t>::find(const tkey& key)
{
    ...
    while (current != nullptr)
    {
        ...
        {
            return bstree_iterator(path, i, this);
        }
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator BS_tree<tkey, tvalue, compare, t>::find(
    const tkey& key) const
{
    ...
    while (current != nullptr)
    {
        ...
        if (i < current->_keys.size() && !compare_keys(key, current->_keys[i].first))
        {
            return bstree_const_iterator(path, i, this);
        }
        ...
        current = current->_pointers[i];
        path.push({const_cast<bstree_node* const*>(&(*path.top().first)->_pointers[i]), i});
    }
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator BS_tree<tkey, tvalue, compare, t>::lower_bound(
    const tkey& key) const
{
    auto it = begin();
    while (it != end() && compare_keys(it->first, key))
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
typename BS_tree<tkey, tvalue, compare, t>::bstree_const_iterator BS_tree<tkey, tvalue, compare, t>::upper_bound(
    const tkey& key) const
{
    auto it = begin();
    while (it != end() && !compare_keys(key, it->first))
...
void BS_tree<tkey, tvalue, compare, t>::clear() noexcept
{
    ...
    std::stack<bstree_node*> nodes_to_delete;
    nodes_to_delete.push(_root);

    while (!nodes_to_delete.empty())
    {
        bstree_node* current = nodes_to_delete.top();
        nodes_to_delete.pop();
        ...
        pp_allocator<bstree_node> node_allocator(_allocator.resource());
...
BS_tree<tkey, tvalue, compare, t>::emplace(Args&&... args)
{
    ...
    {
        pp_allocator<bstree_node> node_allocator(_allocator.resource());
        _root = node_allocator.template new_object<bstree_node>();
        _root->_keys.push_back(std::move(data));
        ++_size;

        std::stack<std::pair<bstree_node**, size_t>> path;
        path.push({&_root, 0});
        return {bstree_iterator(path, 0, this), true};
    }

    bstree_node* current = _root;
    std::stack<bstree_node*> path;
...
template <typename tkey, typename tvalue, comparator<tkey> compare, std::size_t t>
void BS_tree<tkey, tvalue, compare, t>::split_child(bstree_node* node, std::stack<bstree_node*>& path)
    {
        pp_allocator<bstree_node> node_allocator(_allocator.resource());
        bstree_node* new_node = node_allocator.template new_object<bstree_node>();
        ...
        if (path.empty())
        {
            bstree_node* new_root = node_allocator.template new_object<bstree_node>();
...
BS_tree<tkey, tvalue, compare, t>::erase(bstree_iterator pos)
{
    ...
    while (true)
    {
        ...
        if (i < current->_keys.size() && !compare_keys(key, current->_keys[i].first))
        {
            ...
            {
                bstree_node* pred = current->_pointers[i];
